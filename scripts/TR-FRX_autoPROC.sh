...
while [ "$i" -le "$LAST_IMG" ]; do
    ...
    cat > "$JOB_SCRIPT" <<EOF
#!/bin/bash
# NOTE: no 'set -e' here — a failing -ref run must fall through to the standalone retry.
set -uo pipefail
module load autoPROC
cd "$PROCESS_DIR"

REF="$FIRST_OUTDIR/truncate-unique.mtz"

# 1) Primary: process against the shared reference (keeps indexing + scaling consistent).
if process -Id "$RUN_ID,$IMAGES_DIR,$IMAGE_TEMPLATE,$i,$j" -ref "\$REF" AutoProcScale_RunStaraniso=yes -d "$OUTDIR"; then
    exit 0
fi
...
# Re-index this window to the reference frame (POINTLESS tests h,k,l vs -h,-k,l etc.
# against the reference and outputs the matched data). Needs CCP4's pointless.
command -v pointless >/dev/null 2>&1 || module load ccp4 2>/dev/null || true
mkdir -p "$PROCESS_DIR/reports_qc"
for MTZ in "$OUTDIR/truncate-unique.mtz" "$OUTDIR/staraniso_alldata-unique.mtz"; do
    [ -f "\$MTZ" ] || continue
    TMP="\${MTZ%.mtz}.reidx.mtz"
    if pointless hklref "\$REF" hklin "\$MTZ" hklout "\$TMP" \
            > "\${MTZ%.mtz}.reindex_pointless.log" 2>&1 && [ -f "\$TMP" ]; then
        mv "\$TMP" "\$MTZ"
    else
        echo "WARNING: POINTLESS reindex-to-reference failed for \$MTZ (left as-is)" >&2
    fi
done
# Marker: the report flags this window as standalone-reindexed (indexing provenance differs).
printf '%s\treindexed-to-ref\n' "${i}_${j}" >> "$PROCESS_DIR/reports_qc/reindex_standalone.tsv"
echo "  standalone+reindex complete for images $i-$j" >&2
exit 0
EOF
...
#
# The QC pass verifies ITSELF that the exclusion took effect: it compares the
# number of images used in the merge before and after the trim. If the count did
# not drop, `autoPROC_XdsKeyword_EXCLUDE_DATA_RANGE` did nothing in this autoPROC
# build, and the trim is rejected rather than trusted. When the count cannot be
# parsed at all the trim is still accepted on Rmerge/completeness, but the QC log
...
if [ "$QC_ENABLE" -eq 1 ] && [ "${#CHUNK_JOBIDS[@]}" -gt 0 ]; then
    ...
    # --- header with values baked in (expanding heredoc) ---
    cat > "$QC_SCRIPT" <<EOF
#!/bin/bash
# Auto-generated by $(basename "$0") — QC auto-retry for outlier windows.
set -uo pipefail
module load autoPROC
...
REF_MTZ="$FIRST_OUTDIR/truncate-unique.mtz"
QC_RMERGE_ABS="$QC_RMERGE_ABS"
...
cd "$PROCESS_DIR"
mkdir -p "$PROCESS_DIR/reports_qc"
LOG="$PROCESS_DIR/reports_qc/qc_refix.log"
...
# from indexing/integration). Same wording family as the report scraper.
scrape_images(){ # $1 dir -> integer or empty
    local d="$1" tmp f val=""
    tmp=$(mktemp -d)
    [ -f "$d/summary.tar.gz" ] && tar -xzf "$d/summary.tar.gz" -C "$tmp" 2>/dev/null
    while IFS= read -r f; do
        ...
        val=$(grep -hoiE 'number of (images used|batches)[[:space:]]*[:=]?[[:space:]]*[0-9]+' \
                   "$f" 2>/dev/null | grep -oE '[0-9]+' | head -1)
        [ -n "$val" ] && break
...
# chunks sorted by first image.
# NB: a plain while-read loop, not `mapfile` -- mapfile needs bash 4+ and this
# script must also run under the bash 3.2 shipped on macOS.
DIRS=()
while IFS= read -r _d; do
    [ -n "$_d" ] && DIRS+=("$_d")
# The `$` anchor and the grep filter are what keep autoPROC_<i>_<j>.qc_orig_backup
# (written by a previous QC pass) from being picked up as a chunk on a re-run.
done < <(ls -d "$PROCESS_DIR"/autoPROC_*_* 2>/dev/null \
    | sed -E 's#.*/autoPROC_([0-9]+)_([0-9]+)$#\1 &#' \
    | grep -E '^[0-9]+ ' | sort -n | awk '{print $2}')

# Guard before any "${DIRS[@]}" expansion: under `set -u`, bash < 4.4 treats an
# empty array as an unbound variable and would abort here.
if [ "${#DIRS[@]}" -eq 0 ]; then
    say "no autoPROC_<first>_<last> chunk found in $PROCESS_DIR - nothing to QC."
    exit 0
fi
...
for k in "${!DIRS[@]}"; do
    ...
    for T in $QC_TRIMS; do
        estart="$i"; eend=$((i + T - 1))
        [ "$eend" -ge "$j" ] && continue
        tmp="${PROCESS_DIR}/qcfix_${i}_${j}_trim${T}"; rm -rf "$tmp"
        say "  try EXCLUDE_DATA_RANGE=$estart $eend"
        if process -Id "$RUN_ID,$IMAGES_DIR,$IMAGE_TEMPLATE,$i,$j" \
            ...
            if [ -n "$ni" ] && [ -n "$oi" ]; then
                if [ "$(awk -v n="$ni" -v o="$oi" 'BEGIN{print (n<o)?1:0}')" -eq 1 ]; then
                    excl_note="$oi->$ni"
                    say "       exclusion confirmed: images used $oi -> $ni"
                else
                    excl_ok=0
                    say "       REJECTED: images used $oi -> $ni (no reduction);"
                    say "       autoPROC_XdsKeyword_EXCLUDE_DATA_RANGE had no effect"\
                        " on the final merge in this autoPROC build"
                fi
            else
...
say "QC auto-retry finished."
QCEOF
    chmod +x "$QC_SCRIPT"
    QC_DEP=$(IFS=:; echo "${CHUNK_JOBIDS[*]}")
    # afterany (not afterok): the QC pass runs once all chunks TERMINATE, even if
    # some FAILED — so a single bad window (e.g. wrong cell/split crystal) no longer
    # freezes the run with DependencyNeverSatisfied. QC skips windows with no stats;
    # the failed ones are simply reported N/A instead of blocking QC + reports.
    qc_output=$(sbatch -p "$SLURM_PARTITION" -n 1 -c "$SLURM_CPUS" --mem="$SLURM_MEM" \
                --time=06:00:00 --dependency=afterany:"$QC_DEP" "$QC_SCRIPT")
    QC_JOBID=$(echo "$qc_output" | awk '{print $4}')
    echo "  [submitted] QC auto-retry job $QC_JOBID (runs after all ${#CHUNK_JOBIDS[@]} chunks finish, pass or fail)"
fi
...
Written to the processing directory by TR-FRX_autoPROC.sh, which submits it as
the final SLURM job. It also runs standalone, pointed at an existing
``autoproc_chunks`` directory:
...
QC_FIXED = {}   # {"1251_1750": "1251-1470", ...} from reports_qc/qc_fixed.tsv (QCFIX)


def load_qc_fixed(process_dir):
    """Load the QC auto-retry marker: window '<first>_<last>' -> excluded frame range."""
    QC_FIXED.clear()
    path = os.path.join(process_dir, "reports_qc", "qc_fixed.tsv")
    try:
        with open(path) as fh:
            for line in fh:
                p = line.rstrip("\n").split("\t")
                if len(p) >= 4 and p[0]:
                    QC_FIXED[p[0]] = p[3]      # excluded range, e.g. "1251-1470"
    except OSError:
        pass


REINDEXED = set()   # {"1751_1950", ...} from reports_qc/reindex_standalone.tsv (option-A)


def load_reindexed(process_dir):
    """Load the standalone-reindex marker: windows recovered WITHOUT -ref and then
    POINTLESS-reindexed to the reference, so their indexing provenance differs."""
    REINDEXED.clear()
    path = os.path.join(process_dir, "reports_qc", "reindex_standalone.tsv")
    try:
        with open(path) as fh:
            for line in fh:
                p = line.rstrip("\n").split("\t")
...
def _draw_panel(ax, panel, rows, xr, show_repro_bands=True):
    ...
    show_repro_bands: draw the amber QC_FIXED band and the blue hatched
    REINDEXED band (windows that needed a reprocessing exception). Kept in
    the combined PDF report (staraniso/truncate) since the banner text there
    explains them, but left off the standalone per-panel PNG figures,
    which carry no banner and would otherwise show unexplained shading."""
    ...
    for lab, col, pct, ci, dash in specs:
        ...
        drew = True
    if show_repro_bands:
        # QCFIX: shade any window that was QC-reprocessed with fewer images, so its
        # completeness/multiplicity dip is clearly attributed (intentional, not a fault).
        for _qk in QC_FIXED:
            try:
                _qf, _ql = (float(v) for v in _qk.split("_"))
            except (ValueError, AttributeError):
                continue
            ax.axvspan(_qf, _ql, color="#E8A33D", alpha=0.16, lw=0, zorder=0)
        # option-A: shade windows recovered STANDALONE + reindexed to the reference
        # (BLUE, hatched) — their indexing provenance differs from the -ref windows.
        for _rk in REINDEXED:
            try:
                _rf, _rl = (float(v) for v in _rk.split("_"))
            except (ValueError, AttributeError):
                continue
            ax.axvspan(_rf, _rl, facecolor="#3B6FB0", alpha=0.14,
                       hatch="///", edgecolor="#2C5488", lw=0, zorder=0)
    for sp in ("top", "right"):
...
def _damage_plots(pdf, plt, rows, dataset, title, wilson_method=""):
    ...
    # Banner flags (also shaded in every panel below): amber = QC fewer-images;
    # blue hatched = recovered standalone with different indexing. Stack them.
    _banners = []
    if QC_FIXED:
        _qc = "; ".join("%s (excluded frames %s)" % (w, x)
                        for w, x in sorted(QC_FIXED.items()))
        _banners.append(("QC — reprocessed with fewer images (amber bands): " + _qc,
                         "#B45309"))
    if REINDEXED:
        _ri = ", ".join(sorted(REINDEXED))
        _banners.append(("INDEXING DIFFERS — recovered standalone, reindexed to "
                         "reference (blue hatched bands): " + _ri, "#2C5488"))
    _by = htop - 0.032
    for _txt, _col in _banners:
        fig.text(0.07, _by, _txt, fontsize=8, color=_col, fontweight="bold")
        _by -= 0.016
    top_pad = 0.050 + 0.020 * len(_banners)
    xr = _x_range(rows)
    for idx, panel in enumerate(_DAMAGE_PANELS, 1):
        _draw_panel(fig.add_subplot(nrows, 2, idx), panel, rows, xr)
    fig.subplots_adjust(left=0.09, right=0.955, top=htop - top_pad, bottom=0.04,
...
def _damage_figs(plt, rows, out_dir, route, suffix=""):
    """One standalone publication figure per panel — a 600-dpi PNG — written to
    <out_dir>/plots_<route><suffix>/<slug>.png."""
    ...
    for panel in _DAMAGE_PANELS:
        ...
        fig.patch.set_facecolor("white")
        _draw_panel(fig.add_subplot(1, 1, 1), panel, rows, xr, show_repro_bands=False)
        fig.tight_layout()
...
def write_pdf(path, dataset, title, rows, image_summary, wilson_method="",
    ...
    # Publication figures: one 600-dpi PNG per panel, per route.
    if rows and route:
        try:
            sub = _damage_figs(plt, rows, os.path.dirname(path), route, suffix)
            print("  per-panel figures (PNG) -> %s" % sub)
...
def main():
    ...
    load_qc_fixed(process_dir)                      # QCFIX: flag auto-repaired windows
    if QC_FIXED:
        print("QC-repaired windows (excluded frames): "
              + ", ".join("%s[%s]" % (w, x) for w, x in sorted(QC_FIXED.items())))
    load_reindexed(process_dir)                     # option-A: flag standalone-reindexed windows
    if REINDEXED:
        print("Standalone-reindexed windows (indexing differs): "
              + ", ".join(sorted(REINDEXED)))
...
# QCFIX: also wait on the QC auto-retry job (if any) so the report + time_windows.csv
# are generated from the FIXED chunk, not the outlier one.
DEP="afterany:$(IFS=:; echo "${CHUNK_JOBIDS[*]}")"
if [ -n "${QC_JOBID:-}" ]; then DEP="${DEP}:${QC_JOBID}"; fi
report_output=$(sbatch -p "$SLURM_PARTITION" -n 1 -c 1 --mem=8000 --dependency="$DEP" "$REPORT_JOB_SCRIPT")
...
# The QC auto-retry is a real job the report depends on: list it like the rest,
# so the table and the squeue command below account for every submitted job.
if [ -n "${QC_JOBID:-}" ]; then
    printf "  %-12s %s\n" "$QC_JOBID" "QC auto-retry (outlier windows)"
fi
printf "  %-12s %s\n" "$REPORT_JOBID" "final reports"
echo "======================================================="
if [ -n "${QC_JOBID:-}" ]; then
    echo "Total: $(( ${#CHUNK_JOBIDS[@]} + 2 )) jobs (${#CHUNK_JOBIDS[@]} processing + 1 QC + 1 report)"
    TRACK_IDS="$(IFS=,; echo "${CHUNK_JOBIDS[*]}"),$QC_JOBID,$REPORT_JOBID"
else
    echo "Total: $(( ${#CHUNK_JOBIDS[@]} + 1 )) jobs (${#CHUNK_JOBIDS[@]} processing + 1 report)"
    TRACK_IDS="$(IFS=,; echo "${CHUNK_JOBIDS[*]}"),$REPORT_JOBID"
fi
echo ""
echo "Track with: squeue -j $TRACK_IDS"
echo ""
